...
const BASH_DENY_RULES = [
  ...
  { id: "chmod-runtime-security", pattern: /chmod\b.*\/(\.pi\/agent\/extensions\/tool-guard|runtime\/broker-gateway\/security)\./, label: "chmod on protected runtime security file", severity: "block" },
...
describe("tool-guard: source repo protection (bash)", () => {
  ...
  it("blocks chmod on runtime security.mjs", () => {
    assert.equal(checkBashCommand(`chmod 777 ${AGENT_HOME}/runtime/broker-gateway/security.mjs`).blocked, true);
  });
});
...
describe("tool-guard: workspace confinement (allow-list)", () => {
  ...
  it(`allows write to ${AGENT_HOME}/runtime/broker-gateway/bridge.mjs`, () => {
    assert.equal(checkWritePath(`${AGENT_HOME}/runtime/broker-gateway/bridge.mjs`), false);
  });
...
describe("tool-guard: source repo is fully read-only (write/edit)", () => {
  ...
  it("blocks write to source repo bridge", () => {
    assert.equal(checkWritePath(`${BAUDBOT_SOURCE_DIR}/broker-gateway/bridge.mjs`), true);
  });
});
...
describe("tool-guard: protected runtime security files", () => {
  ...
  it("blocks write to runtime security.mjs", () => {
    assert.equal(checkWritePath(`${AGENT_HOME}/runtime/broker-gateway/security.mjs`), true);
  });
  it("blocks write to runtime security.test.mjs", () => {
    assert.equal(checkWritePath(`${AGENT_HOME}/runtime/broker-gateway/security.test.mjs`), true);
  });
