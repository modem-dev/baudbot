...
if [[ ! "$(uname -s)" =~ Linux ]]; then
  die "Hornet requires Linux (kernel-level isolation). macOS/Windows are not supported."
fi

echo ""
echo -e "${BOLD}🐝 Hornet Installer${RESET}"
echo ""
...
if [ "$DISTRO" = "unknown" ]; then
  die "Unsupported distro. Hornet is tested on Ubuntu 24.04 and Arch Linux."
fi
...
if [ -z "$ADMIN_USER" ] || [ "$ADMIN_USER" = "root" ]; then
  ...
  while IFS=: read -r username _ uid _ _ home _; do
    if [ "$uid" -ge 1000 ] && [ "$uid" -lt 60000 ] && [ -d "$home" ] && [ "$username" != "hornet_agent" ]; then
      ADMIN_USER="$username"
      break
    fi
  done < /etc/passwd
...
if [ -n "$SCRIPT_DIR" ] && [ -f "$SCRIPT_DIR/setup.sh" ] && [ -f "$SCRIPT_DIR/bin/deploy.sh" ]; then
  ...
  if [ -d "$REPO_DIR/.git" ]; then
    ...
    REPO_URL="https://github.com/modem-dev/hornet.git"
...
ENV_FILE="$HORNET_HOME/.config/.env"

echo -e "Hornet needs API keys to talk to services. You'll be prompted for each one."
...
prompt_secret "HORNET_EMAIL" \
  "Agent email address (e.g. agent@agentmail.to)"

if [ -n "${ENV_VARS[AGENTMAIL_API_KEY]:-}" ]; then
  prompt_secret "HORNET_SECRET" \
    "Email auth secret (or press Enter to auto-generate)"
  if [ -z "${ENV_VARS[HORNET_SECRET]:-}" ]; then
    ENV_VARS[HORNET_SECRET]="$(openssl rand -hex 32)"
    dim "  Auto-generated: ${ENV_VARS[HORNET_SECRET]}"
  fi

  prompt_secret "HORNET_ALLOWED_EMAILS" \
...
# Always set these
ENV_VARS[HORNET_AGENT_USER]="hornet_agent"
ENV_VARS[HORNET_AGENT_HOME]="$HORNET_HOME"
ENV_VARS[HORNET_SOURCE_DIR]="$REPO_DIR"
...
# Build .env content
ENV_CONTENT="# Hornet agent configuration
# Generated by install.sh on $(date -Iseconds)
# Edit with: sudo -u hornet_agent vim $ENV_FILE
"
...
chown hornet_agent:hornet_agent "$ENV_FILE"
chmod 600 "$ENV_FILE"
...
if [ -n "$MISSING" ]; then
  ...
  echo -e "  ${DIM}sudo -u hornet_agent ~/runtime/start.sh${RESET}"
  echo ""
else
  ask "Start the agent now? [Y/n]: "
  read -r launch
  if [ -z "$launch" ] || [[ "$launch" =~ ^[Yy] ]]; then
    info "Launching in tmux session 'hornet'..."
    sudo -u hornet_agent tmux new-session -d -s hornet "$HORNET_HOME/runtime/start.sh" 2>/dev/null || true
    sleep 2
    if sudo -u hornet_agent tmux has-session -t hornet 2>/dev/null; then
...
SSH_PUB="$HORNET_HOME/.ssh/id_ed25519.pub"

echo -e "🐝 ${BOLD}Hornet is installed.${RESET}"
echo ""
echo -e "  ${BOLD}Edit secrets:${RESET}    sudo -u hornet_agent vim $ENV_FILE"
echo -e "  ${BOLD}Start agent:${RESET}     sudo -u hornet_agent ~/runtime/start.sh"
echo -e "  ${BOLD}Attach to tmux:${RESET}  sudo -u hornet_agent tmux attach -t hornet"
