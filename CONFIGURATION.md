...
`start.sh` runs `varlock load` to validate before launching — the agent won't start with missing or malformed variables — and then launches the control-agent itself via `varlock run` (rather than sourcing `.env`). Rooting the launch chain at `varlock run` means pi and every descendant `varlock run` (the gateway bridge, dev/sentry subagents) inherit varlock's nested-resolution marker, so each re-resolves config from `~/.config/.env` on restart instead of honoring stale values inherited from the long-lived control-agent. The gateway bridge is likewise launched under `varlock run`, giving it the same schema validation and fresh-on-restart resolution.
