...
make_manifest() {
  ...
  local bridge_file="$release_dir/broker-gateway/bridge.mjs"
  ...
  cat >"$manifest_file" <<EOF
{
  ...
  "files": {
    ...
    "release/broker-gateway/bridge.mjs": "$(hash_file "$bridge_file")",
...
mkdir -p "$HOME1/.pi/agent/extensions" "$HOME1/runtime/bin" "$HOME1/.pi/agent/logs" "$RELEASE1/broker-gateway"

printf 'console.log("ok");\n' > "$HOME1/.pi/agent/extensions/test.ts"
printf '#!/bin/bash\necho helper\n' > "$HOME1/runtime/bin/helper.sh"
printf 'export const bridge = true;\n' > "$RELEASE1/broker-gateway/bridge.mjs"
